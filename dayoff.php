...
<head>
	...
	<!-- Title -->
	<title>Dayoff და შვებულებები</title>
	<!---Fontawesome css-->
...
<body class="main-body main-sidebar-hide">
		...
		<div class="main-content side-content pt-0">
			...
			<div class="container-fluid">
				...
				<div class="page-header">
					<div>
						<h2 class="main-content-title tx-24 mg-b-5">Dayoff და შვებულებები</h2>
						<ol class="breadcrumb">
							<li class="breadcrumb-item"><a href="#">პარამეტრები</a></li>
							<li class="breadcrumb-item active" aria-current="page">Dayoff და შვებულებები</li>
						</ol>
					</div>
				</div>
				<!-- End Page Header -->
	...
	$(document).on("dblclick", "#users tr.k-state-selected", function () {
		...
		$.ajax({
			...
			success: function(data){
                ...
                $("#vacation_type,#user_id,#user_social").chosen();
				GetDate('start_date');
                GetDate('end_date');
	...
	$(document).on('click','#button_trash',function(){
		...
		$.ajax({
			url: aJaxURL,
			type: "POST",
			data: "act=disable_d&id=" + removeIDS,
			dataType: "json",
			success: function (data) {
	...
	function LoadKendoTable_incomming(hidden){
		...
		const kendo = new kendoUI();
		kendo.loadKendoUI(aJaxURL,'get_list_dayoff',itemPerPage,columnsCount,columnsSQL,gridName,actions,editType,columnGeoNames,filtersCustomOperators,showOperatorsByColumns,selectors,hidden, 1, locked, lockable);
	...
	function save_category(){
		...
		params.act 			= 'save_dayoff';
		params.id 			= $("#vacation_id").val();
		params.vacation_type 	= $("#vacation_type").val();
