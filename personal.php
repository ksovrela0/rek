...
<body class="main-body main-sidebar-hide">
	...
	<div title="გათავისუფლება" id="get_layoff_page">
			...
			if(fileSize>20971520) {
				...
				$.ajax({
					...
					success: function (data) {
						...
						if(data.status == 'OK'){
							$("#upProdImg").attr("src", data.src);
							$("#avatar_del").css('display','block');
							alert("სურათი ატვირთულია");
						}
						else{
							alert('ვერ მოხერხდა სურათის ატვირთვა');
						}
					}
				});
			}
	...
	$(document).on('click','#avatar_del', function(){
		$.ajax({
			url: aJaxURL,
			type: "POST",
			data: "act=delete_file&type=avatar&id=" + $("#user_id").val(),
			dataType: "json",
			success: function (data) {
	...
	$(document).on('click','#button_lay_off',function(){
			var removeIDS = [];
			...
			if(removeIDS.length == 1){
				if(confirm("ნამდვილად გსურთ მონიშნული თანამშრომლის გათავისუფლება?")){
					$.ajax({
						url: aJaxURL,
						type: "POST",
						data: {
						...
						dataType: "json",
						success: function(data){
							$("#get_layoff_page").html(data.page);
							$("#get_layoff_page").dialog({
								resizable: false,
								height: 500,
								width: 600,
								modal: true,
								buttons: {
									"გათავისუფლება": function() {
										var layoff_reason = $("#layoff_reason").val();
										var layoff_order_file = $("#layoff_order_file").val();
										var ready_to_layoff = 0;
										if(layoff_reason == ''){
											ready_to_layoff++;
											alert("გთხოვთ შეიყვანოთ კომენტარი");
										}

										if(layoff_order_file == ''){
											ready_to_layoff++;
											alert("გთხოვთ ატვირთოთ ბრძანება");
										}


										if(ready_to_layoff == 0){
											$.ajax({
												url: aJaxURL,
												type: "POST",
												data: {
				...
				alert("გასათავისუფლებლად უნდა მონიშნოთ მხოლოდ 1 თანამშრომელი");
			}
